...
public final class Geometry {
	 ...
	 * 
	 * @param x0 Upper left x (inclusive)
         * @param y0 Upper left y (inclusive)
         , @param x1 Lower right x (inclusive)
         * @param y1 Lower right y (inclusive)
	...
	/**
	 * Find the point on the line p0,p1 [x,y,z] a given fraction from p0. Fraction
	 * of 0.0 whould give back p0, 1.0 give back p1, 0.5 returns midpoint of line
	 * p0,p1 and so on. F raction can be >1 and it can be negative to return any
	 * point on the line specified by p0,p1.
	 * 
	 ...
	 * 
	 * @param x0 x coordinate of first point defining the line
	 * @param y0 y coordinate of first point defining the line
	 * @param x1 x coordinate of second point defining the line
	 * @param y1 y coordinate of second point defining the line
	 ...
	 * 
	 * @param p0 first point of line segment to extend [x,y,z].
         * @param p1 second point of line segment to extend [x,y,z].
	 * @param toLength Length of new line segment.
	 * @param anchor Specifies the fixed point during extension. If anchor is 0.0,
	 *        p0 is fixed and p1 is adjusted. If anchor is 1.0, p1 is fixed and p0
	 ...
	 * 
	 * @param p0 first point of segment to extend. Fixed during extension.
           @param p1 second point of line segment to extend.
	 ...
	 * 
	 * @param p0 first point to compute distance between [x,y,z].
           @param p1 second point to compute distance between [x,y,z].
	 ...
	 * 
	 * @param x0 x coordinate of first line end point.
         * @param y0 y coordinate of first line end point.
	 * @param x1 x coordinate of second line end point.
         * @param y1 y coordinate of second line end point.
	...
	public static double length(int[] x, int[] y, boolean isClosed) {
        ...
        /** Return squared distance between two points (for comparison)
        @param p0 first point
        @param p1 second point
	 ...
	 * point (x,y). Ref: http://astronomy.swin.edu.au/pbourke/geometry/pointline/
	 ...
	 * @return <0 if points on opposite sides. =0 if one of the points is exactly
	 *         on the line >0 if points on same side.
	 */
	public static int sameSide(double x0, double y0, double x1, double y1, double px0, double py0, double px1, double py1) {
		int sameSide = 0;
	 ...
	 *         on the line >0 if points on same side.
	 */
	public static int sameSide(int x0, int y0, int x1, int y1, int px0, int py0, int px1, int py1) {
	 ...
	 * @param zExtent Will upon return contain [xMin,xMax]. Unused (may be set to
	 *        null) if z is null.
	 */
	 ...
	 * @return -1 if lines are parallel (x,y unset), -2 if lines are parallel and
	 *         overlapping (x, y center) 0 if intesrection outside segments (x,y
	...
	public static double[] findLinePolygonIntersections(double[] x, double[] y, double x0, double y0, double x1, double y1) {
		...
		double[] intersections = new double[100]; // Result vector x,y,x,y,...
	 ...
	 * task, and then transforms this according to any rotation or skew defined by
	 * the given top points.
	 * 
	...
	public static double[] createEllipse(double x0, double y0, double dx, double dy) {
		...
		// we create one Point per 2nd degree. The nPoints variable holds
	...
	/**
	 * Convert an int array into a double array
	 * 
	 * @param anArray the int array to convert
	 * @return the returned double array copied in from the int array
	 */
	public static double[] toDoubleArray(int[] anArray) {
		double[] d = new double[anArray.length];
	...
	public static boolean intersects(Point aStart, Point anEnd, Collection<Polygon> somePolygons) {
		for (Polygon p : somePolygons) {
			if (Geometry.intersects(aStart, anEnd, p)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Does this line intersect this polygon?
	 * 
	 * @return true if it does, otherwise false;
	 * 
	 */
	public static boolean intersects(Point aStart, Point anEnd, Polygon aPolygon) {
		if (aPolygon.contains(aStart) || aPolygon.contains(anEnd)) {
			return true;
		}
		PolygonPathVisitor line = new PolygonPathVisitor(aPolygon);
		int[] segLine = line.getCurrentSegment();
		while (line.hasNext()) {
			if (findLineSegmentIntersection(aStart.x, aStart.y, anEnd.x, anEnd.y, segLine[0], segLine[1], segLine[2], segLine[3], new double[6]) == 1) {
				return true;
			}
			segLine = line.nextSegment();
		}
		if (findLineSegmentIntersection(aStart.x, aStart.y, anEnd.x, anEnd.y, segLine[0], segLine[1], segLine[2], segLine[3], new double[6]) == 1) {
			return true;
		}
	...
	public static Point contractPoint(Point aPoint, Point aReference) {
		Point p = new Point();
		if (aReference.x <= aPoint.x) {
			p.x = aPoint.x - 1;
		} else {
			p.x = aPoint.x + 1;
		}
		if (aReference.y <= aPoint.y) {
			p.y = aPoint.y - 1;
		} else {
			p.y = aPoint.y + 1;
		}
	...
	public Polygon scalePolygon(Polygon aPolygon, double aScaleX, double aScaleY) {

		double[] data = getData(aPolygon); // cx, cy, R
		int sides = aPolygon.xpoints.length;
		// double x = (1.0 - scaleX) * data[0];
		// double y = (1.0 - scaleY) * data[1];
		// AffineTransform at = AffineTransform.getTranslateInstance(x, y);
		// at.scale(scaleX, scaleY);
		// Shape shape = at.createTransformedShape(polygon);
		int R = (int) (((aScaleX + aScaleY) / 2) * data[2]);
		int[][] xy = generateShapeArrays((int) data[0], (int) data[1], R, sides);
		return new Polygon(xy[0], xy[1], sides);
	}

	private double[] getData(Polygon polygon) {
		Rectangle2D r2 = polygon.getBounds2D();
		double cx = r2.getX() + r2.getWidth() / 2;
		double cy = r2.getY() + r2.getHeight() / 2;
		int sides = polygon.xpoints.length;
		// find length of a side
		double s = Point2D.distance(polygon.xpoints[0], polygon.ypoints[0], polygon.xpoints[1], polygon.ypoints[1]);
		// find circumradius
		double R = s / (2 * Math.sin(Math.PI / sides));
		// find inradius
		double r = R * Math.cos(Math.PI / sides);
		// find distance from center of bounding rectangle
		// to center of outer/inner circle
		double dy = (R - r) / 2;
		return new double[] { cx, cy + dy, R };
	}
	...
	public double radiansToDegrees(double someRadians) {
		return someRadians * (180 / Math.PI);
	}

	public double degreesToRadians(double someDegrees) {
		return someDegrees * (Math.PI / 180);
	}
